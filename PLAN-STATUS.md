...
| B4 | Local ASR dependency/model proof | **Harness/runbook verified; live quality benchmark deferred** | Python 3.12.3 ASR venv, faster-whisper 1.2.1, ffmpeg 8.1.2, and tiny model (CPU/int8) were proven on 2026-07-04 and re-smoked offline against the synthetic silence-only fixture on 2026-07-05. Full dependency receipts, safe re-verification commands, and benchmark gating are recorded in `docs/dev/local-runbook.md`. Real-word-speed proof on owned media remains deferred until an exact approved owned clip path or approved selection rule is provided. |
